 - The function init the grid `grid = [[None]*3, [None]*3, [None]*3]` is wrong, as it create same list values for rows. Change it to: `grid = [[None for _ in range(3)] for _ in range(3)]`
