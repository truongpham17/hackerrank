...
var MyCalendarTwo = function () {
  this.list = new BinarySearchTree((a, b) => a[0] > b[0])
  this.join = new BinarySearchTree((a, b) => a[0] > b[0])
};
...
MyCalendarTwo.prototype.book = function (start, end) {
  const largestSmaller = this.join.findLargestSmaller([end, null])
  if (largestSmaller && largestSmaller[1] > start) {
    return false
  } else {
    // checking here
    const intersect = this.list.inorder(undefined, [start, end], [])
    this.list.insert([start, end])
    for (const inter of intersect) {
      this.join.insert(inter)
    }
    return true 
  }
};
